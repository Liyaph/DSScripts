...
categories <- cut(scores, breaks=c(0, 70, 86, 100), right = F, include.lowest = TRUE, labels = c("Poor", "Good", "Excellent"))
...
```{r}
categories.f <- factor(x=categories)

scores[categories.f == "Good"]
```
...
```{r}
class$modes[class$modes == "Online"]
```
